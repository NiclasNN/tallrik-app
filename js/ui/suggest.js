...
import { state, dayTotals, addEntry, commit, save, addShopping, toggleShopping, removeShopping, clearShoppingDone, clearShopping } from '../store.js?v=202609141620';
import { esc, fmt, haptic, toast, openSheet, confirmSheet, MEALS, guessMeal, todayKey, segmented, bindSeg, closeAllSheets, cute } from '../components.js?v=202609141620';
import { I } from '../icons.js?v=202609141620';
import { RECIPES, RESEARCH, suggestMeals, weightLossIndex, indexReasons, violatesAvoid } from '../recipes.js?v=202609141620';
import { aiStatus, suggest as aiSuggest, ask as aiAsk, addKey, PROVIDERS, planDay } from '../ai.js?v=202609141620';
import { weightTrend } from '../nutrition.js?v=202609141620';
...
export function render(root) {
  ...
  root.innerHTML = `<div class="page">
    ...
    ${ai.textConfigured ? `<div class="section">
      <div class="section-head"><h2 class="h2">Nya recept åt dig</h2><button class="link-btn" id="ai-new" ${aiBusy ? 'disabled' : ''}>${I.refresh} Nya</button></div>
      <button class="small muted" id="research" style="margin:0 4px 10px;text-align:left;color:var(--accent)">🔬 Så väljer Tallrik – forskningen bakom förslagen →</button>
    ...
    <div class="section">
      <div class="section-head"><h2 class="h2">Utforska recept</h2><span class="small muted">TheMealDB · öppet API</span></div>
      <div class="card">
        ...
        <p class="small faint mt-2">Öppna receptdata från TheMealDB (bild, ingredienser, instruktioner, YouTube). AI:n översätter och räknar näringen när du öppnar ett recept.</p>
      </div>
    </div>

    ${ai.textConfigured ? `<div class="section">
      <div class="section-head"><h2 class="h2">Fråga coachen</h2></div>
      <div class="card">
...
export function openDayPlan(rem, onChange = null) {
  ...
  async function generate(api, wishText = '') {
    ...
    try {
      ...
      const plan = await planDay({ kcal: Math.max(600, t.kcal - tot.kcal), protein: Math.max(30, t.protein - tot.protein), focus, wish: wishText, slots: slots.length ? slots : daySlots, library: c.aiRecipes.slice(0, 40).map(r => ({ title: r.title, slot: r.slot, kcal: r.kcal, protein: r.protein })) });
      ...
      c.dayPlan = { date: today, ...plan }; commit('cache'); haptic('success');
    } catch (e) { toast(e.message, 'bad'); }
    busy = false; draw(api);
  }
  function draw(api) {
    ...
    api.setHTML(`${busy ? `<div class="card"><div class="row" style="gap:12px"><span class="spinner"></span><div><div class="h3">Planerar din dag…</div><div class="small muted">Recept för resten av dagen: ${fmt(Math.max(600, t.kcal - dayTotals(today).kcal))} kcal och ${fmt(Math.max(30, t.protein - dayTotals(today).protein))} g protein kvar</div></div></div><div class="skeleton mt-2" style="height:90px"></div><div class="skeleton mt-1" style="height:90px"></div><div class="skeleton mt-1" style="height:90px"></div></div>` : ''}
      ${plan && !busy ? `<div class="card accent"><div class="row between"><div><div class="eyebrow">${eatenTot.kcal ? 'Ätit + planerat' : 'Hela dagen'}</div><div class="hero-num">${fmt(sum.kcal + eatenTot.kcal)} <small style="font-size:14px;color:var(--text-3)">/ ${fmt(t.kcal)} kcal</small></div>${eatenTot.kcal ? `<div class="small muted">varav ${fmt(eatenTot.kcal)} redan ätit</div>` : ''}</div><div class="small muted" style="text-align:right">P ${fmt(sum.protein + eatenTot.protein)} / ${fmt(t.protein)} g<br>K ${fmt(sum.carbs)} · F ${fmt(sum.fat)} g</div></div>${plan.note ? `<div class="small mt-2" style="color:var(--accent)">${esc(plan.note)}</div>` : ''}</div>
      ...
      ${!plan && !busy ? '<div class="notice">Ingen plan ännu.</div><button class="btn primary block mt-3" id="replan">Planera min dag</button>' : ''}`);
...
async function openMealDB(m0) {
  ...
  const draw = (busy = false) => {
    ...
    a.body.querySelector('#adapt')?.addEventListener('click', async () => {
      draw(true);
      try {
        const { adaptMeal } = await import('../ai.js?v=202609141620');
