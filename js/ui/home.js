...
import { state, dayTotals, day, setWater, latestWeight, streak, photoURL } from '../store.js?v=202609141620';
import { esc, fmt, ring, animateIn, macroBars, MEALS, meal as mealCfg, greeting, todayKey, addDays, fmtDate, haptic, initials, countUp, timeHM, guessMeal, weightSpark, isCute, mascot, cute } from '../components.js?v=202609141620';
import { I } from '../icons.js?v=202609141620';
import { foodEmoji, fmtSize } from '../foods.js?v=202609141620';
import { aiStatus } from '../ai.js?v=202609141620';
import { weightTrend } from '../nutrition.js?v=202609141620';
import * as Log from './log.js?v=202609141620';
import { pickForSlot, recipeImage, openRecipe, logRecipeAs } from './suggest.js?v=202609141620';
...
export function render(root) {
  ...
  const slots = state.profile.mealsPerDay === 2 ? ['lunch', 'dinner', 'snack'] : ['breakfast', 'lunch', 'dinner', 'snack'];
  ...
  root.innerHTML = `<div class="page">
    <div class="row between">
      ...
      <div class="row gap-1">
        ...
        <button class="icon-btn accent" id="quick-photo" aria-label="Fota maten" title="Fota maten">${I.camera}</button>
    ...
    <div class="row between mt-2" style="gap:6px">
      ...
      <button class="btn sm ghost grow" id="day-today" ${isToday ? 'disabled' : ''}>${isToday ? 'Idag' : 'Gå till idag'}</button>
      <label class="icon-btn" style="position:relative;overflow:hidden" aria-label="Välj datum">📅<input type="date" id="day-pick" value="${viewDate}" max="${todayKey()}" style="position:absolute;inset:0;opacity:0;width:100%;height:100%"></label>
    ...
    ${state.recents.length ? `<div class="section" style="margin-top:14px"><div class="section-head"><h2 class="h2">Snabbval</h2><span class="small muted">ett tryck → portion</span></div><div class="chips">${state.recents.slice(0, 8).map((r, i) => `<button class="chip sm" data-quick="${i}">${foodEmoji(r)} ${esc(r.name)}${r.lastGrams ? ` · ${fmtSize(r.lastGrams, r.liquid)}` : ''}</button>`).join('')}</div></div>` : ''}
  ...
  root.querySelector('#quick-photo').addEventListener('click', () => { haptic(); Log.startPhotoFlow(guessMeal(), viewDate, { capture: true }); });
...
function mealCard(id, tot, t) {
  ...
  return `<div class="card meal">
    <div class="meal-head">
      ...
      <button class="icon-btn accent round" data-add-meal="${id}" aria-label="Lägg till i ${esc(m.name)}">${I.plus}</button>
    </div>
    ${bm.entries.length ? `<div class="entries">${bm.entries.map(entryRow).join('')}</div>` : `<div class="empty-line">${cute('Inget loggat ännu', id)}</div>`}
  </div>`;
}
function entryRow(e) {
  const sub = [e.portionLabel && e.source === 'photo' ? e.portionLabel : e.grams ? `${fmt(e.grams)} ${e.liquid ? 'ml' : 'g'}` : (e.portionLabel || ''), e.brand || '', e.source === 'photo' ? '📸' : '', timeHM(e.time)].filter(Boolean).join(' · ');
