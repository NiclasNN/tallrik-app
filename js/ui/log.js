...
import { state, addEntry, updateEntry, removeEntry, day, savePhoto, enqueueAnalysis, toggleFavorite, isFavorite, pushRecent, addCustomFood, removeCustomFood, addWeight, latestWeight, rememberCorrection, deletePhoto } from '../store.js?v=202609141620';
import { esc, fmt, openSheet, toast, haptic, MEALS, guessMeal, todayKey, dateKey, addDays, fmtDate, segmented, bindSeg, confirmSheet, confColor, uid, sleep, debounce, closeAllSheets, weightSpark } from '../components.js?v=202609141620';
import { I } from '../icons.js?v=202609141620';
import { loadDB, search, scale, portionPresets, foodEmoji, lookupBarcode, searchProducts, anchorSearch, fmtSize, packageSizes, productTitle } from '../foods.js?v=202609141620';
import { pickPhoto, compressImage, canLiveScan, startScanner, decodeBarcodeFromImage } from '../camera.js?v=202609141620';
import { analyzePhoto, aiStatus } from '../ai.js?v=202609141620';
import { navigate } from '../app.js?v=202609141620';
...
export function openAddSheet(meal = guessMeal(), date = todayKey()) {
  const recents = state.recents.slice(0, 8);
  openSheet({
    title: 'Logga mat',
    html: `${mealSeg(meal)}
      <div class="action-grid mt-3">
        <button class="action-tile hero" data-act="photo" style="flex-direction:row;align-items:center;gap:14px"><div class="ic">${I.camera}</div><div><div class="t">Fota maten</div><div class="d">Tallrik eller förpackning – AI:n läser etiketten, räknar portionen och hämtar exakta värden.</div></div></button>
        <button class="action-tile" data-act="library"><div class="ic amber">${I.image}</div><div class="t">Från album</div><div class="d">Bild du redan tagit – loggas på rätt dag</div></button>
        <button class="action-tile" data-act="scan"><div class="ic blue">${I.barcode}</div><div class="t">Skanna streckkod</div><div class="d">Exakt produkt via Open Food Facts</div></button>
        <button class="action-tile" data-act="search"><div class="ic green">${I.search}</div><div class="t">Sök livsmedel</div><div class="d">2 600 svenska livsmedel + produkter</div></button>
        <button class="action-tile" data-act="quick" style="grid-column:1/-1;flex-direction:row;align-items:center"><div class="ic pink">${I.bolt}</div><div><div class="t">Snabbt</div><div class="d">Bara kalorier, till exempel från en meny</div></div></button>
      </div>
      ${recents.length ? `<div class="eyebrow mt-3" style="margin-bottom:8px">Snabbval – dina senaste</div><div class="chips">${recents.map((r, i) => `<button class="chip sm" data-recent="${i}">${foodEmoji(r)} ${esc(r.name)}${r.lastGrams ? ` · ${fmtSize(r.lastGrams, r.liquid)}` : ''}</button>`).join('')}</div>` : ''}`,
    onMount(api) {
      let m = meal;
...
// ---------- Foto ----------
export async function startPhotoFlow(meal = guessMeal(), date = todayKey(), { capture = true } = {}) {
  const file = await pickPhoto({ capture });
  if (!file) return;
  let img;
  try { img = await compressImage(file, 1280, 0.82); } catch { toast('Kunde inte läsa bilden', 'bad'); return; }
  const photoId = uid();
...
export function showResults(a, res, { img, photoId, meal, date, takenAt = null }) {
  const items = res.items.map(it => ({ ...it, include: true, id: uid() }));
  let curMeal = meal, curDate = date, added = false;
  ...
  const guard = () => { if (added || !items.some(i => i.include)) return true; confirmSheet({ title: 'Kasta analysen?', text: 'Måltiden är inte tillagd ännu.', okText: 'Kasta', danger: true }).then(ok => { if (ok) a.close(null, true); }); return false; };
  ...
  const draw = () => {
    ...
    a.setHTML(`
      <div class="row" style="align-items:flex-start">
        <img src="${img.dataURL}" alt="" style="width:64px;height:64px;border-radius:14px;object-fit:cover;flex:0 0 auto">
        <div class="grow"><div class="small muted">${esc(res.notes || 'Kontrollera portionerna – tryck på en rad för att justera.')}</div>
        <div class="row gap-1 mt-1" style="flex-wrap:wrap"><span class="tag ai">${esc(res.model || 'AI')}</span><span class="tag">${Math.round((res.ms || 0) / 1000)} s</span><span class="tag good">${srcCount}/${items.length} exakta källor</span>${pkgCount ? `<span class="tag">${pkgCount} förpackning${pkgCount > 1 ? 'ar' : ''}</span>` : ''}</div></div>
      </div>
      <div class="mt-2" id="items">
        ${items.length ? items.map(row).join('') : `<div class="notice">Ingen mat hittades på bilden. Prova att fota rakt uppifrån i bra ljus, eller sök manuellt.</div>`}
      </div>
      ${items.length ? `<div class="chips mt-2" id="scale"><span class="small muted" style="align-self:center;margin-right:2px">Alla portioner:</span><button class="chip sm" data-scale="0.5">Hälften</button><button class="chip sm" data-scale="0.75">−25 %</button><button class="chip sm" data-scale="1.25">+25 %</button><button class="chip sm" data-scale="2">Dubbelt</button></div>` : ''}
      <div class="row mt-2" style="gap:8px">
        <button class="btn sm grow" id="add-more">${I.plus} Lägg till fler</button>
        <button class="btn sm grow" id="fix">${I.edit} Rätta AI:n</button>
      </div>
      <div id="fixbox" class="hidden mt-2">
        <div class="field"><input class="input" id="fix-text" placeholder="T.ex. det är en 50 cl-flaska · kalkon, inte kyckling · halv portion"></div>
        <button class="btn primary block mt-1" id="fix-run">Analysera igen med rättelsen</button>
      </div>
      <div class="divider"></div>
      <div class="eyebrow" style="margin-bottom:8px">Måltid</div>
      ${mealSeg(curMeal)}
      ...
      <div class="eyebrow mt-3" style="margin-bottom:8px">Dag</div>
      <div class="chips" id="days">${[todayKey(), addDays(todayKey(), -1), ...(curDate !== todayKey() && curDate !== addDays(todayKey(), -1) ? [curDate] : [])].map(k => `<button class="chip sm ${k === curDate ? 'active' : ''}" data-day="${k}">${esc(fmtDate(k))}</button>`).join('')}<label class="chip sm" style="position:relative;overflow:hidden">Annan dag…<input type="date" id="day-pick" max="${todayKey()}" style="position:absolute;inset:0;opacity:0;width:100%"></label></div>
      <div class="row between mt-3" style="padding:0 4px"><div><div class="hero-num" id="tot-kcal">${fmt(t.kcal)} <small style="font-size:14px;color:var(--text-3)">kcal</small></div>${macroLine(t)}</div></div>
      <button class="btn primary block mt-2" id="add-all" ${items.filter(i => i.include).length ? '' : 'disabled'}>Lägg till ${fmt(t.kcal)} kcal</button>
      <button class="btn quiet block mt-1" id="discard">Kasta analysen</button>`);
    ...
    a.body.querySelector('#fix-run').onclick = () => { const h = a.body.querySelector('#fix-text').value.trim(); if (!h) return; added = true; a.close(null, true); openAnalysisSheet({ img, photoId, meal: curMeal, date: curDate, hint: h, takenAt }); };
    a.body.querySelector('#discard').onclick = () => { if (!guard()) return; deletePhoto(photoId); a.close(null, true); };
    a.body.querySelector('#add-all').onclick = () => {
      ...
      for (const it of items.filter(i => i.include)) {
        const e = addEntry(curDate, { meal: curMeal, name: it.name, grams: it.grams, kcal: it.kcal, protein: it.protein, carbs: it.carbs, fat: it.fat, per100: it.per100, source: 'photo', photoId: ids.length === 0 ? photoId : null, confidence: it.confidence, match: it.match?.name || '', group: it.match?.group || '', foodKey: it.match?.key || '', emoji: it.match ? foodEmoji(it.match) : foodEmoji({ name: it.name }), image: it.image || '', portionLabel: it.type === 'package' ? pkgLabel(it) : '', liquid: it.liquid });
      ...
      added = true; haptic('success'); closeAllSheets();
  ...
  const row = it => {
    const sub = it.type === 'package'
      ? `${esc(pkgLabel(it))}${it.sizeRead ? '' : ' <span style="color:var(--accent-2)">?</span>'} · ${esc(it.sourceLabel)}`
      : `≈ ${fmt(it.grams)} g${it.grams_range && it.grams_range[0] !== it.grams_range[1] ? ` (${fmt(it.grams_range[0])}–${fmt(it.grams_range[1])})` : ''} · ${esc(it.sourceLabel || 'AI-skattning')}`;
    const icon = it.image ? `<img src="${esc(it.image)}" alt="" style="width:40px;height:40px;border-radius:10px;object-fit:cover;flex:0 0 auto">` : `<div style="width:40px;height:40px;border-radius:10px;background:var(--surface-2);display:grid;place-items:center;font-size:20px;flex:0 0 auto">${it.type === 'package' ? (it.liquid ? '🥤' : '📦') : foodEmoji(it.match || { name: it.name })}</div>`;
    return `<button class="ai-item" data-item="${it.id}" style="width:100%;text-align:left;opacity:${it.include ? 1 : .45}">${icon}
      <div class="grow"><div class="n"><span class="conf" style="background:${confColor(it.confidence)}"></span>${esc(it.name)}${it.hidden ? ' <span class="tag">dolda kalorier</span>' : ''}${it.learned ? ' <span class="tag good">lärt</span>' : ''}</div>
  ...
  function openItemSheet(it) {
    ...
    openSheet({
      ...
      html: `<div class="field"><label>Namn</label><input class="input" id="it-name" value="${esc(it.name)}"></div>
        ${it.basis ? `<div class="small muted mt-1" style="margin-left:4px">AI: ${esc(it.basis)}</div>` : ''}
        ${isPkg ? `
        ...
        <div class="chips mt-2" id="g-chips">
          <button class="chip sm" data-g="${it.grams_range[0]}">Låg ${fmt(it.grams_range[0])} g</button>
          <button class="chip sm" data-g="${Math.round((it.grams_range[0] + it.grams_range[1]) / 2)}">Mitt ${fmt(Math.round((it.grams_range[0] + it.grams_range[1]) / 2))} g</button>
          <button class="chip sm" data-g="${it.grams_range[1]}">Hög ${fmt(it.grams_range[1])} g</button>
          <button class="chip sm" data-g="${Math.round(it.grams / 2)}">Halv</button><button class="chip sm" data-g="${it.grams * 2}">Dubbel</button>
        </div>`}
        <div class="eyebrow mt-3" style="margin-bottom:8px">Näringsvärden från</div>
        ...
        ${it.alternatives?.length ? `<div class="eyebrow mt-3" style="margin-bottom:8px">Kan det vara…</div><div class="chips">${it.alternatives.map((alt, i) => `<button class="chip sm" data-alt="${i}">${esc(alt)}</button>`).join('')}</div>` : ''}
        <div class="col mt-4"><button class="btn primary block" id="it-save">Klar</button><button class="btn danger block" id="it-del">Ta bort från måltiden</button></div>`,
      onMount(sa) {
        const b = sa.body;
        let part = it.part || 1;
        const upd = () => {
        ...
        b.querySelector('#it-save').onclick = () => { it.name = b.querySelector('#it-name').value.trim() || it.name; upd(); sa.close(); draw(); haptic(); };
