...
import { state } from './store.js?v=202609141620';
import { round } from './components.js?v=202609141620';
...
export function search(q, { limit = 40, includeCustom = true } = {}) {
  let nq = norm(q);
  if (!nq) return [];
  for (const [k, v] of SYN) if (nq.includes(k)) nq = nq.replace(k, v);
...
const NAME_EMOJI = [
  [/\bris\b|couscous|bulgur|quinoa/i, '🍚'], [/potatis|pommes|mos\b/i, '🥔'], [/ägg\b/i, '🥚'], [/kyckling|kalkon/i, '🍗'],
  [/lax|torsk|fisk|räk|tonfisk|sill|makrill|skaldjur/i, '🐟'], [/korv/i, '🌭'], [/kött|biff|färs(?![a-zåäö])|fläsk|skinka|bacon|lamm/i, '🥩'], [/banan/i, '🍌'], [/äpple/i, '🍎'], [/apelsin|clementin|citrus/i, '🍊'],
  [/jordgubb|hallon|blåbär|bär\b/i, '🍓'], [/avokado/i, '🥑'], [/morot|broccoli|spenat|gurka|tomat|paprika|grönsak|sallat|kål/i, '🥦'], [/yoghurt|kvarg|skyr|filmjölk|\bfil\b|keso/i, '🥛'],
  [/ost\b|cheddar|mozzarella|feta|halloumi/i, '🧀'], [/kaffe|latte|cappuccino/i, '☕'], [/\bte\b|grönt te/i, '🍵'], [/\böl\b|lager|ipa/i, '🍺'], [/\bvin\b|rosé|prosecco/i, '🍷'], [/choklad|kakao/i, '🍫'],
  [/glass/i, '🍦'], [/bröd|knäcke|limpa/i, '🍞'], [/bulle|kaka|tårta|muffin|kex/i, '🍰'], [/nöt|mandel|jordnöt|cashew/i, '🥜'], [/smoothie|juice/i, '🥤'], [/bönor|linser|kikärt|tofu|falafel/i, '🌱'],
];
...
export function rankProducts(list, query, sizeGrams = null) {
  ...
  return list.map(p => {
    ...
    let s = 0;
    for (const t of qt) { if (pn.includes(t)) s += 10; else if (bn.includes(t)) s += 4; }
    if (qt.length && qt.every(t => all.includes(t))) s += 15;
...
export function anchorSearch(q, limit = 4) {
  ...
  for (const f of DB) {
    let s = 0, matched = 0;
    for (const t of toks) {
      if (f._n.startsWith(t)) { s += 30; matched++; }
      else if (f._w.some(w => w.startsWith(t))) { s += 18; matched++; }
      else if (t.length >= 4 && f._n.includes(t.slice(0, -1))) { s += 8; matched++; }
    }
    if (!matched) continue;
