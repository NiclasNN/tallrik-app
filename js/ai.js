...
import { state, dequeueAnalysis, getPhoto, blobToDataURL, addEntry, commit, getCorrection, activeUserId, clearSharedKey, mergeSharedKeys } from './store.js?v=202609141620';
import { toast, IS_LOCAL_HOST, fixEscapes } from './components.js?v=202609141620';
import { loadDB, matchItem, scale, lookupBarcode, searchProductsByBrand, parseQuantity, packageSizes, fmtSize, productTitle } from './foods.js?v=202609141620';
...
function healthPenalty(model) { const h = aiSettings().health?.[model]; if (!h?.lastFail) return 0; return Date.now() - Date.parse(h.lastFail) < 30 * 60 * 1000 ? Math.min(80, 30 * (h.fails || 1)) : 0; }
function noteFailure(model) { const a = aiSettings(); a.health ||= {}; const h = a.health[model] || { fails: 0 }; h.fails = (Date.now() - Date.parse(h.lastFail || 0) < 30 * 60 * 1000 ? h.fails : 0) + 1; h.lastFail = new Date().toISOString(); a.health[model] = h; commit('settings'); }
...
// ---- Leverantörsanrop ----
async function generate({ provider, key, model, prompt, imageDataURL = null, json = true, maxTokens = 2048, temperature = 0.2 }) {
  ...
  if (provider === 'gemini') {
    ...
    const mv = ver(model);
    if (/flash/.test(model) && !/lite/.test(model)) { if (mv >= 3) gen.thinkingConfig = { thinkingLevel: aiSettings().mode === 'precision' ? 'medium' : 'low' }; else if (mv >= 2.5) gen.thinkingConfig = { thinkingBudget: aiSettings().mode === 'precision' ? 1024 : 0 }; }
    let j;
    ...
    try { j = await http(url, { method: 'POST', headers, body }); }
      ...
      if (e.code === 'http' && /reasoning_format/i.test(e.message)) { delete body.reasoning_format; j = await http(url, { method: 'POST', headers, body }); }
...
async function run(ctxFn, opts) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try { const r = await generate({ ...ctx, model, ...opts }); r.model = model; return r; }
      ...
      if (e.code === 'server' || e.code === 'timeout' || e.code === 'ratelimit' || e.code === 'parse' || e.code === 'empty') { noteFailure(model); continue; }
      throw e;
    }
  }
...
Användarens egen kommentar (väger tungt om den finns): "${String(hint || '').replace(/"/g, "'").slice(0, 400)}"

Svara med ENBART ett JSON-objekt enligt exakt denna form (null där något inte gäller):
{"meal_title":"kort svensk titel","items":[{"type":"package","name":"Coca-Cola","db_query":"Läsk cola sockrad","state":"","grams":330,"grams_range":[330,330],"count":1,"package":{"brand":"Coca-Cola","product":"Coca-Cola Original","variant":"","declared_size":"33 cl","container":"burk","size_read_from_label":true},"ean":"5449000000996","label_per100":{"kcal":42,"protein":0,"carbs":10.6,"fat":0},"kcal":139,"protein":0,"carbs":35,"fat":0,"confidence":0.9,"basis":"33 cl läst på burken; näring från deklarationen","alternatives":[],"hidden":false},{"type":"food","name":"Lax stekt","db_query":"Lax stekt","state":"stekt","grams":150,"grams_range":[120,180],"count":1,"package":null,"ean":null,"label_per100":null,"kcal":300,"protein":30,"carbs":0,"fat":20,"confidence":0.8,"basis":"kort motivering","alternatives":["Regnbåge stekt"],"hidden":false}],"notes":"kort kommentar om osäkerheter"}`;
}
...
export async function analyzePhoto(dataURL, { hint = '', meal = '', onStage = () => {}, extraEan = null } = {}) {
  const { provider, key, model } = current();
  onStage('identifying');
  const [res] = await Promise.all([run(current, { prompt: analyzePrompt(hint, meal), imageDataURL: dataURL, maxTokens: 3500 }), loadDB().catch(() => null)]);
  onStage('matching');
  let ean = null;
...
export async function normalizeAnalysis(j, { model = '', ms = 0, extraEan = null } = {}) {
  ...
  for (const raw of rawItems) {
    ...
    const it = {
      ...
      ai, aiPer100: per100FromGrams(ai, grams), package: null, ean: cleanEan(raw.ean), sizeRead: false, unitGrams: 0, liquid: null, sizes: [], sourceLabel: '',
    };
    if (isPkg) {
    ...
    if (product) {
      ...
      const { chosen, candidates } = matchItem(it.db_query, ai.kcal, grams);
...
export async function suggest(b) {
  ...
  const res = await run(currentText, { prompt: suggestPrompt(b), maxTokens: Math.min(7000, 1200 + 1100 * Math.min(6, Math.max(3, +b.count || 3))), temperature: 0.7 });
