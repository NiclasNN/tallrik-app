...
// ---- Toast ----
export function toast(msg, kind = '', ms = 2600, { action = '', onAction = null } = {}) {
...
export function confirmSheet({ title, text, okText = 'Ja', danger = false }) {
    ...
    const s = openSheet({
      title, html: `<p class="lead" style="margin-bottom:18px">${esc(text)}</p>
        <div class="col gap-1">
          ...
          <button class="btn block quiet" data-cancel>Avbryt</button>
        </div>`,
      onMount(api) {
